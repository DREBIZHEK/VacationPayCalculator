...
class VacationPayCalculatorApplicationTests {
	double salary;
	String vacationStart, vacationEnd;

	@Before
	public void setUp() {
		salary = 3102.5;
		vacationStart = "05.10.2022";
		vacationEnd = "18.10.2022";
	}

	@Test
	public void CalculateTest(){
		double result = 1020;
		assertEquals("Wrong calculations",result,Calculate(salary,vacationStart,vacationEnd));
	}
	...
	public double Calculate(double salary, String vacationStart, String vacationEnd) {
		...
		if (vacationStart.matches(regex) && vacationEnd.matches(regex) && salary>0){
			...
			return salaryPerDay*WeekendCount(CreateDate(vacationStart),CreateDate(vacationEnd));
		}
		else
			return -1;
	}
